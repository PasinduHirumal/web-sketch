...
import AdminFilterMenu from '@/components/common/AdminFilterMenu';
        ...
        <div className="p-4 max-w-7xl mx-auto w-full animate-fade-in">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                <div>
                    ...
                    <p className="text-gray-500 mt-2 text-sm">View, edit roles, and manage system user accounts.</p>
                </div>
                ...
                <div className="flex items-center gap-3 w-full md:w-auto">
                    <form onSubmit={handleSearch} className="relative flex-1 sm:flex-initial sm:w-64 md:w-72">
                        ...
                        <input
                            ...
                            className="w-full pl-10 pr-4 py-2 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none text-gray-900 transition-shadow shadow-sm text-sm h-[38px]"
                        />
                    </form>
                    ...
                    <button
                        ...
                        type="button"
                        className={`flex items-center justify-center p-2 rounded-xl border border-gray-200 bg-white hover:bg-gray-50 text-gray-700 transition shadow-sm h-[38px] w-[38px] cursor-pointer relative ${roleFilter || statusFilter || sortOrder !== "DESC"
                            ? "ring-2 ring-blue-500/50 border-blue-500 text-blue-600"
                            : ""
                            }`}
                        title="Filter Users"
                    >
                                    ...
                                    <tr key={u.id} className="hover:bg-gray-50 transition-colors group">
                                        <td className="py-4 px-6">
                                            <div className="flex items-center gap-3">
                                                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-100 to-blue-200 flex items-center justify-center text-blue-700 font-bold text-sm shrink-0 shadow-inner">
                                                ...
                                                <div className="flex flex-col">
                                                    <span className="font-semibold text-gray-900 flex items-center gap-2 text-sm">
                                                        {u.name}
                                                        {u.id === currentUser?.id && (
                                                            <span className="text-[10px] uppercase tracking-wider bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full font-bold">
                                                                You
                                                            </span>
                                                        )}
                                                    </span>
                                        ...
                                        <td className="py-4 px-6">
                                            {isSuperAdmin(currentUser?.role) && u.id !== currentUser?.id ? (
                                                <select
                                                    ...
                                                    className="bg-gray-50 hover:bg-gray-100 border border-gray-200 text-gray-900 text-sm rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 block py-1.5 pl-3 pr-8 transition-colors cursor-pointer appearance-none disabled:opacity-50"
                                                ...
                                                <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${u.role === UserRole.SUPER_ADMIN
                                                    ? 'bg-purple-100 text-purple-700 ring-1 ring-purple-600/10'
                ...
                {!loading && totalPages > 1 && (
                    <div className="border-t border-gray-100 px-6 py-4 flex flex-col sm:flex-row items-center justify-between gap-4 bg-gray-50/30">
                        <span className="text-sm text-gray-500 font-medium">
                            Showing <span className="text-gray-900 font-semibold">{page}</span> of <span className="text-gray-900 font-semibold">{totalPages}</span> ({totalItems} total users)
                        </span>
                        <div className="flex items-center gap-2">
                            <button
                                disabled={page === 1}
                                onClick={() => setPage(p => p - 1)}
                                className="px-4 py-2 text-sm font-semibold text-gray-700 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-sm active:scale-[0.98]"
                            >
                                Previous
                            </button>
                            <button
